...
object GpsRecordingSettings {
    ...
    fun videoMode(context: Context): GpsRecordingMode = read(context, KEY_VIDEO_MODE, GpsRecordingMode.Off)
    fun audioMode(context: Context): GpsRecordingMode = read(context, KEY_AUDIO_MODE, GpsRecordingMode.Off)
    ...
    private fun read(context: Context, key: String, fallback: GpsRecordingMode): GpsRecordingMode {
        val stored = context.applicationContext.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            .getString(key, fallback.name)
        return GpsRecordingMode.entries.firstOrNull { it.name == stored } ?: fallback
    }
